...
export default function FactoriesError({
  error,
  reset
}: {
  ...
  return (
    <main className="min-h-screen bg-[color:var(--background)] px-4 py-10">
      <section className="mx-auto max-w-2xl rounded-md border border-rose-200 bg-white p-5 shadow-sm">
        <p className="text-sm font-semibold text-rose-700">공장 목록을 불러오지 못했습니다.</p>
        <h1 className="mt-2 text-2xl font-semibold text-[color:var(--foreground)]">잠시 후 다시 시도해 주세요.</h1>
        <p className="mt-3 text-sm leading-6 text-[color:var(--muted)]">
          Supabase 공개 조회 또는 네트워크 상태를 확인해야 합니다. 이전 화면으로 돌아가거나 같은 요청을 다시 실행할 수 있습니다.
        </p>
        {error.message ? (
